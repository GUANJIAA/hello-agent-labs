...
# 默认ReAct提示词模板
DEFAULT_REACT_PROMPT = """你是一个具备推理和行动能力的AI助手。你可以通过思考分析问题，然后调用合适的工具来获取信息，最终给出答案。
...
**Thought:** 分析当前问题，思考需要什么信息或者采取什么行动。
**Action:** 选择一个行动，格式必须是以下之一:
- `{{tool_name}}[{{tool_input}}]` - 调用指定工具
- `Finish[{{answer}}]` - 当你有足够信息给出最终答案时

## 重要提醒
1. 每次回应必须要包含Thought和Action两部分
...
class ReActAgent(Agent):
    ...
    def add_tool(self, tool): 
        ...
        """
        # 检查是否是MCP工具
        if hasattr(tool, 'auto_exapand') and tool.auto_expand:
            # MCP工具会自动展开多个工具
            if hasattr(tool, '_available_tools') and tool._available_tools:
                ...
                print(f"MCP工具'{tool.name}' 已展开为 {len(tool._available_tools)} 个独立的工具")
            else:
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        Args:
            input_text: 输入文本
            kwargs: 其他参数（如max_steps、system_prompt等）
        ...
        if len(preview) > 160:
            preview = preview[:160] + "..."
        print("\n" + hr('=', 80))
        print(c(f"{self.name}", PRIMARY) + " " + c(preview, INFO))
        print(hr('=', 80))
        ...
        while current_step < self.max_steps:
            ...
            if not action:
                ...
                try:
                    ...
                    repair_user = f"Rewrite the following into the required two-line form:\n{response_text}"
            ...
            if action.startswith("Finish"):
                ...
                self.add_message(Message(thought, "assistant"))
            ...
            if not tool_name or not tool_input:
                self.current_history.append("Observation: 无效的Action格式, 请检查输入")
                continue
            ...
            if self.early_stop_on_repeat:
                ...
                if repeat_count >= self.repeat_action_threshold:
                    self.current_history.append("Observation: 重复相同Action, 无明显进展, 终止")
                    break
